...
MongoClient.connect('mongodb://localhost:27017/users', function(err, db) {
	...
	dir.paths(PIC_INPUTFOLDER, function(err, paths) {
	    if (err) throw err;
	    	...
	    	q.drain = function() {
	    	    console.log('all items have been resized');
	    	};




	    	var qq = async.queue(function (file, callback) {
	    			    			    ...
	    			    			    new ExifImage({ image : file }, function (error, exifData) {
	    			    			        if (error)
	    						        	    ...
	    						        	    fs.readFile(file, function (err, data) {
	    						        			...
	    						        			if ('GPSLatitudeRef' in exifData.gps) exifData.gps.latlon = get_lat_lon(exifData.gps);
	    	...
	    	qq.drain = function() {
	    		db.close();
	    	    console.log('all items have been added to db');
	    	};
	    	...
	    	if (filePath[filePath.length-4] === path.basename(PIC_INPUTFOLDER)) {
	    		...
	    		if (setting_json.exclude_directorys.indexOf(collectionName) === -1) {
	    			...
	    			if (path.extname(file).toUpperCase() === ('.JPG' || '.JPEG' )) {
	    				...
	    				q.push(file); // dispatching doc to async.queue
	    				qq.push(file); // dispatching doc to async.queue
	    			}
	    		}
	    	}
	    });
